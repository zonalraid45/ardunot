# FULL UPDATED FILE WITH NEW RATE LIMITS
...
# Channel that always chats/responds – special limit 100 msg/min
...
RATE_LIMITS = {
    "serious": 10,   # si = 10 msg/min
    "funny": 20      # fi = 20 msg/min
}

BOT_CHANNEL_LIMIT = 100  # ALWAYS_CHANNEL_ID special limit
...
def can_send_in_guild(guild_id: int, mode: str, channel_id: int) -> bool:
    """
    Updated: special bot channel = 100 msg/min
    Other channels depend on mode (serious=10, funny=20)
    """
    ...
    # special limit for bot-chat channel
    if channel_id == ALWAYS_CHANNEL_ID:
        limit = BOT_CHANNEL_LIMIT
    else:
        limit = RATE_LIMITS.get(mode, RATE_LIMITS["funny"])
